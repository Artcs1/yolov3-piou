...
def gauss2xywh(x,scale=0.25):

    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[2] = (torch.sqrt(y[2]))/scale
    y[3] = (torch.sqrt(y[3]))/scale

    return y

def bbox_piou(box1, box2, x1y1x2y2=False, hellinger = True):

    """
        box1: predicted box 
        box2: annotated box

        x1y1x2y2 = True  => box representation x,y,w,h     (center, width, height)
        x1y1x2y2 = False => box representation x1,y1,x2,y2 (corners) --- need to implement
    ...
    box1 = xywh2gauss(box1)
    box2 = xywh2gauss(box2)

    b1_x1, b1_y1, b1_a1, b1_b1 = box1[0], box1[1], box1[2], box1[3]
    b2_x2, b2_y2, b2_a2, b2_b2 = box2[0], box2[1], box2[2], box2[3]


    t1 = ((torch.pow(b1_x1 - b2_x2,2)/(b1_a1 + b2_a2)) + (torch.pow(b1_y1 - b2_y2,2)/(b1_b1 + b2_b2)))/2.0

    t2 = torch.log((b1_a1+b2_a2)*(b1_b1+b2_b2))/2.0

    t3 = torch.log(b1_a1*b1_b1*b2_a2*b2_b2)/4.0

    t4 = torch.log(torch.tensor(2.0).cuda())

    db = t1 + t2 - t3 - t4

    if hellinger==True:
        H = torch.sqrt(1-torch.exp(-db))
        db = 1 - H
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            #giou = bbox_piou(pbox.t(), tbox[i], x1y1x2y2=False, hellinger)
            ...
            if hellinger == False:
                ...
                tobj[b, a, gj, gi] = (model.gr) + model.gr * piou.detach().clamp(0).type(tobj.dtype)            
            else:
            ...
            #lbox += (piou).sum() if red == 'sum' else (piou).mean()  # giou loss
